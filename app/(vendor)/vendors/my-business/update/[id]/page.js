...
const VendorUpdate = () => {
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit, isSubmitting, setSubmitting, setFieldValue } = useFormik({
        ...
        onSubmit: async () => {
            ...
            values.businessLogo ? formData.append('businessLogo', values.businessLogo) : formData.append('existBusinessLogo', vendor.business_logo);

            //   else{
            //     formData.append('existingLogo',vendor.business_logo)
            //   }
            businessImages?.forEach((img) => {
                if (img.file) {
                    formData.append('businessImage[]', img.file);
                }
            })
            const existingBusinessImage = businessImages.filter((image) => !image.file).map((image) => image.url)

            existingBusinessImage.forEach((url) => {
                formData.append('existBusinessImage[]', url);
            });

            const user_token = localStorage.getItem('authorization');
            console.log('vendor authorization', formData);
            await axios.post(
                `${process.env.NEXT_PUBLIC_API_URL_SYSTEM}/vendorUpdate/${params.id}`,
                formData,
                {
                ...
                if (response.status == 202) {
                    ...
                    router.push('/vendors/my-business')
                }
            }).catch((error) => {
    ...
    useEffect(() => {
        if (vendor.business_logo) {
            setLogoPreview(vendor.business_logo);
        }
        if (vendor.business_images) {
            const images = vendor.business_images.map(url => ({ id:url.id,url:url.business_image, file: null }));
            console.log('business images',images );
    ...
    const handleLogoChange = (e) => {
        ...
        if (file) {
            ...
            console.log('businessLogo value:', values.businessLogo);
        };
    };
    ...
    const removeBusinessImage = (deleteId) => {
        setBusinessImages(prev =>
    ...
    return (
        <div className="bg-gray-100 min-h-screen flex items-center justify-center text-black">
            <div className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-4xl">
                <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">Vendor Update</h2>
                <form onSubmit={handleSubmit} method="post" encType="multipart/form-data" className="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" onChange={handleChange} onBlur={handleBlur} value={values.businessName} name="businessName" required className="mt-1 p-2 w-full rounded-lg border-gray-300 shadow-sm" />
                        {touched.businessName && errors.businessName ? <p className='text-sm text-red-500'>{errors.businessName}</p> : ""}
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700">Category</label>
                        <select name="category" onChange={handleChange} value={values.category} onBlur={handleBlur} required className="mt-1 px-2 py-1 w-full rounded-lg border-gray-300 shadow-sm">
                    ...
                    <div>
                        ...
                        <textarea onChange={handleChange} value={values.description} onBlur={handleBlur} name='description' className="mt-1 p-3 overflow-hidden w-full rounded-lg border-gray-300 shadow-sm"></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            value={query ? query : values.location}
                            onChange={(e) => {
                    ...
                    <button
                        ...
                        className={`w-full mt-6 font-semibold py-2 px-4 rounded-lg shadow-md text-white ${isSubmitting
